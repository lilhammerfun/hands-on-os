...
这里最容易产生的误解是：不同进程里“同类的段”是不是会放在一起。答案是否定的。每个进程都有自己的段表，段号只在当前进程自己的地址空间里有意义。进程 A 可以把段号 0 当作代码段，进程 B 也可以把段号 0 当作代码段，但 A 的代码段基址可能是 0x10000，B 的代码段基址可能是 0x90000。同样的虚拟地址 `(0, 0x1234)`，在 A 里会翻译到 `0x11234`，在 B 里则会翻译到 `0x91234`。所以段内偏移不是因为“同类段被放到一起”才存在，而是因为只知道“这是代码段”还不够，还必须知道“这是代码段里的哪个位置”。
...
可以先抓住分页最核心的直觉：虚拟页和物理页帧之间不要求一一按顺序对应。虚拟页 0 可以映射到物理页帧 5，虚拟页 1 可以映射到物理页帧 2。程序看到的连续虚拟地址，在物理内存里完全可以打散存放。现在来看它的工程细节。
