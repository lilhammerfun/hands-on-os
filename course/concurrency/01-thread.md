...
上一章讨论的是进程。我们已经看到，操作系统可以把执行流隔离成彼此独立的地址空间，用 `fork`、信号、进程组、调度和命名空间管理它们的创建、协作与隔离。现在视角继续往前推：如果问题不再是"多个进程怎样共存"，而是"同一个进程内部怎样并发执行"，上一章那套以隔离为前提的模型就不够用了。

来看这样一个场景：一个 Web 服务器要同时处理多个客户端请求。用多进程可以做到：每来一个请求，fork 一个子进程处理。但每个进程有独立的地址空间，进程之间要共享数据（比如缓存、连接池、计数器），就得用管道或共享内存这样的 IPC 机制，开销不小。而且 fork 要复制页表，虽然有 COW 优化，但后续写入会触发缺页异常、分裂物理页。进程间上下文切换还要刷 TLB。
...
并发这一章会顺着这条主线继续展开。本课先建立**线程**这个新的执行单位，说明它和进程的根本差别。执行流一旦开始共享地址空间，正确性问题就立刻出现，于是下一课进入**同步原语**。同步如果使用不当，又会把系统带进**死锁**。等共享数据的正确性讲清楚之后，我们再看另一条路线：不靠阻塞线程，而靠 **事件驱动并发** 组织大量 I/O。最后回到内核内部，看 **内核同步机制** 怎样在不能随便睡眠、不能随便阻塞的环境里解决同类问题。
