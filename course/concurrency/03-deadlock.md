...
上一课的同步原语解决了竞态条件，但锁本身会带来新的问题。来看一个最简单的例子——两个线程，两把锁，获取顺序相反：
...
四个条件的"必要"可以用反证法验证：每去掉一个条件，死锁就不可能形成。

**去掉互斥**。如果 lock_a 可以被多个线程同时持有，那 thread2 在时刻 4 请求 lock_a 时直接成功，不会阻塞，没有死锁。但 mutex 的语义就是互斥，去掉它等于去掉了锁本身。并非所有资源都需要互斥访问，比如只读数据可以共享，但对于需要互斥的资源（如写操作），这个条件是内在的，无法去掉。

**去掉持有并等待**。如果线程必须一次性获取所有需要的锁，thread1 在获取 lock_a 之前就必须同时请求 lock_a 和 lock_b。如果 lock_b 不可用，thread1 不会持有 lock_a，thread2 就能正常获取 lock_a，不会形成循环等待。代价是：线程必须提前知道自己需要哪些锁，而且一次性锁住所有资源会降低并发度。

**去掉非抢占**。如果系统可以抢占锁，当 thread1 等待 lock_b 超时后，系统强制 thread2 释放 lock_b，死锁就被打破了。但强制释放锁意味着 thread2 的临界区操作被中断，共享数据可能处于不一致的状态。所以锁的抢占在实践中很少使用。

**去掉循环等待**。如果规定所有线程必须按 lock_a → lock_b 的顺序获取锁，thread2 就不能先获取 lock_b 再获取 lock_a。两个线程都先获取 lock_a，只有一个能成功，另一个阻塞在 lock_a 上，但此时阻塞的线程没有持有任何锁，不满足"持有并等待"，死锁不会发生。这是实践中最常用的预防策略。
:::
...
T1 持有 lock_a（lock_a → T1 的分配边），请求 lock_b（T1 → lock_b 的请求边）。T2 持有 lock_b（lock_b → T2 的分配边），请求 lock_a（T2 → lock_a 的请求边）。沿着边的方向走：T1 → lock_b → T2 → lock_a → T1，形成了一个环。
...
上一课结尾介绍的哲学家就餐问题也可以用 RAG 分析。五个哲学家 P0-P4，五根筷子 C0-C4。每个哲学家先拿左边的筷子（Pi 请求 Ci），再拿右边的筷子（Pi 请求 C(i+1)%5）。如果所有哲学家都拿起了左边的筷子，RAG 中就形成了一个五节点的环：P0 → C1 → P1 → C2 → P2 → C3 → P3 → C4 → P4 → C0 → P0。每根筷子是单实例资源，有环即死锁。

:::expand 多实例资源的资源分配图

前面说"有环则必定死锁"只对单实例资源成立。如果一类资源有多个实例，有环不一定死锁。

考虑这个例子：资源 R 有 2 个实例，线程 T1 和 T2 各持有一个实例，线程 T3 正在请求 R。
...
T3 请求 R，R 的两个实例分别被 T1 和 T2 持有，所以 T3 必须等待。但只要 T1 或 T2 中的任何一个释放了它持有的实例，T3 就可以获取，不会死锁。这里虽然可以画出 T3 → R → T1 的路径，但不构成死锁，因为 T1 没有在等待 T3 持有的任何资源。
...
操作系统处理死锁有四种策略：预防(prevention)、避免(avoidance)、检测(detection)和恢复(recovery)。
...
用一个具体例子推演。假设系统有 2 类资源（A 和 B），A 有 10 个实例，B 有 5 个实例。3 个进程 P0、P1、P2，各自声明了最大需求：
...
1. P1 还需要 (1, 2)，不超过 (5, 4)，可以满足。假设 P1 执行完毕并释放所有资源，可用变为 (5+2, 4+0) = (7, 4)
2. P0 还需要 (7, 4)，不超过 (7, 4)，可以满足。P0 执行完毕，可用变为 (7+0, 4+1) = (7, 5)
3. P2 还需要 (6, 0)，不超过 (7, 5)，可以满足

安全序列 〈P1, P0, P2〉 存在，系统处于安全状态。

现在 P2 请求额外的资源 (1, 0)。银行家算法模拟分配后的状态：P2 的已分配变为 (4, 0)，还需要变为 (5, 0)，可用变为 (4, 4)。重新检查是否存在安全序列。P1 还需要 (1, 2) ≤ (4, 4)，可以执行，释放后可用变为 (6, 4)。P0 还需要 (7, 4) > (6, 4)，A 不够。P2 还需要 (5, 0) ≤ (6, 4)，可以执行，释放后可用变为 (10, 4)。P0 还需要 (7, 4) ≤ (10, 4)，可以执行。安全序列 〈P1, P2, P0〉 存在，所以这次分配是安全的，银行家算法允许分配。

银行家算法需要每个进程提前声明最大资源需求，这在实际系统中很难做到。而且每次资源请求都要执行安全性检查，时间复杂度为 O(m × n²)（m 是资源类型数，n 是进程数），开销不小。所以银行家算法在实际操作系统中几乎不使用，但它是理解"安全状态"概念的最佳教学工具。

**检测：wait-for graph 与 lockdep。** 既然预防和避免都有各自的限制，另一种思路是：允许死锁发生，但通过算法检测它。

对于单实例资源，检测死锁等价于在 wait-for graph（等待图）中找环。wait-for graph 是资源分配图的简化版：去掉资源节点，如果线程 Ti 在等待线程 Tj 持有的资源，就画一条边 Ti → Tj。图中有环就意味着死锁。
...
**有竞态条件但没有数据竞争**：两个线程各自加锁后检查账户余额并转账。每次内存访问都有锁保护，没有数据竞争。但如果两个线程同时检查到余额 100 元，各自转出 80 元，最终余额变成 -60 元。程序正确性依赖于执行顺序（应该一个先转完另一个再检查），这是竞态条件。
...
| 预防(prevention) | 设计时破坏四个条件之一，最实用的是全局锁序 |
| 避免(avoidance) | 运行时检查安全状态，银行家算法是经典方案 |
| 检测(detection) | wait-for graph 找环，lockdep 在锁使用模式中检测锁序违反 |
| 恢复(recovery) | 终止进程或资源抢占 |
| 活锁(livelock) | 线程持续执行但无有效进展，随机退避可解决 |
| 原子性违规 | check-then-act 模式，假设原子但未保护 |
| 顺序违规 | 假设操作顺序但未用同步机制保证 |
| 数据竞争(data race) | 并发访问同一位置、至少一个写、无同步 |

**核心洞察**：死锁的四个必要条件是分析和解决的基础。预防死锁就是破坏其中一个条件，而在工程实践中，给所有锁规定获取顺序（破坏循环等待）是唯一兼顾正确性和可行性的策略。Linux 内核通过 lockdep 在运行时自动验证这个顺序，把一个需要程序员自觉遵守的规范变成了机器可检查的约束。
