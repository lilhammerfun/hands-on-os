...
// include/linux/pid.h
struct upid {
    int nr;                     // PID value in this namespace layer
    struct pid_namespace *ns;   // corresponding namespace
};
...
struct pid {
    ...
    struct upid numbers[];      // flexible array: one upid per namespace layer
};
```
...
struct pid {
    level = 2,
    numbers[0] = { nr = 54321, ns = root namespace },       // host sees PID 54321
    numbers[1] = { nr = 89,    ns = middle namespace },    // middle layer sees PID 89
    numbers[2] = { nr = 1,     ns = innermost namespace }, // self sees PID 1
};
```

`getpid()` 返回 `numbers[level].nr`，即进程所在最深层 namespace 的 PID 值。宿主机通过 `numbers[0].nr` 看到它。同一个进程，不同的数字，不同的视角。这个设计很漂亮：不是给进程一个"真实 PID"加一堆别名，而是让每一层 namespace 都有平等的地位。
...
veth（virtual ethernet）设备总是成对创建，如同一根虚拟网线的两端。数据从一端进去，从另一端出来。把一端放进容器的 Network namespace，另一端留在宿主机，就建立了容器和宿主机之间的网络通道：
...
```
0 1000 1        # UID 0 inside namespace maps to UID 1000 outside
```
...
close(fd);
// current process is now in PID 1234's Network namespace
```
