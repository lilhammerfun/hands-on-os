...
- 写作时间：`2026-02-27 首次提交，2026-03-30 最近修改`
- 当前字符：`24032`
...
再看 `ls | grep .zig`，两个命令同时运行，`ls` 的输出直接流入 `grep` 的输入。要理解这件事，得先看 shell 怎样做**重定向**，再看两个进程怎样通过**管道**连起来。

这三步加上连接它们的机制，就是本篇的全部内容。先有**进程**这个管理单位，内核才能记录"谁在运行、谁是谁的父子、谁占了哪些资源"。系统要把用户空间拉起来，就必须先启动**第一个用户态进程**，也就是 PID 1。shell 自己也是普通进程，所以它要启动新命令，仍然要通过 **fork** 复制出一个子进程，再用 **exec** 把这个副本替换成目标程序。目标程序结束后，父进程必须用 **wait** 回收它，否则系统里就会留下僵尸。进程不仅要被创建和回收，还要接到终端、文件和其他进程上，这就引出了**重定向与管道**。
...
exec 家族的命名也很有 Unix 风格：函数名里的每个字母都在说明参数形式和行为。
...
| 函数 | 特点 |
|------|------|
| `execve` | 最底层版本，直接传路径、`argv` 和 `envp` |
| `execvp` | 传 `argv`，并按 PATH 搜索命令 |
| `execvpe` | 同时支持 `argv`、PATH 搜索和显式 `envp` |
...
到这里，fork → exec → wait 三步走完了。进程的生命周期本身并不复杂。但一个进程不仅要被创建和回收，还要接到终端、文件和其他进程上。shell 执行 `ls | grep foo` 时，`ls` 的输出怎么流到 `grep` 的输入？这就需要重定向和管道。
...
**`dup(old_fd)`** 复制 old_fd 所指向的内核文件对象，并让**当前最小空闲**槽位也指向它。上图里如果执行 `dup(3)`，返回的就是 fd 4；执行后，fd 3 和 fd 4 都指向 `/home/user/a.txt`。

**`dup2(old_fd, new_fd)`** 做的是同一件事，但目标槽位由调用者显式指定。上图里 `dup2(3, 4)` 的效果就是：让原本 `(free)` 的 fd 4 也指向 `/home/user/a.txt`。如果 fd 4 原来不是 free，内核会先把它关掉，再让它指向和 fd 3 相同的内核文件对象。

注意这里复制的不是“文件内容”，也不是重新打开一次文件，而是**让两个 fd 槽位指向同一个内核文件对象**。这也是为什么它们会共享文件偏移量和状态。

于是重定向可以写成一个更典型的 shell 版本。exec 一节的 `ls > output.txt`，子进程里大致会做这几步：

```
// pseudocode
fork()
// in the child process:
fd = open("output.txt", ...) // open the target file
dup2(fd, 1)                  // make stdout (fd 1) point to the same file
close(fd)                    // original fd is now redundant
exec("ls")                   // ls writes to fd 1, which now goes to output.txt
```

如果只是把 stdout 重定向到一个**新打开的文件**，`close(1) + open(...)` 当然也能工作，因为 `open()` 总会占用最小空闲 fd，最后自然拿到 fd 1。但 `open + dup2` 更通用也更直观：管道、`2>&1`、以及“把任意 fd 接到任意目标上”这些场景，最终都要落回 `dup2` 这套模型。

ls 完全不知道自己的输出被重定向了。它照常 `write(1, ...)` 写 fd 1，但 fd 1 已经不是终端而是文件。这里再次体现了 Unix 设计的优雅：程序不需要知道输出去了哪里，fd 抽象屏蔽了一切细节。

当 shell 不再把 stdout 接到文件，而是要把一个进程的输出接到另一个进程的输入上时，上面这套 fd 机制就进一步发展成了管道。关键并没有变：仍然是让某几个 fd 槽位改接到新的内核对象上。

管道(pipe)是一个内核维护的单向字节缓冲区，通过两个文件描述符访问：一个只能写（写端），一个只能读（读端）。写端写入的数据在读端按相同顺序读出。

它之所以能跨进程工作，前提是 fork 会把 fd 表复制给子进程。fork 时，子进程得到父进程 fd 表的**副本**。两张表初始内容相同，但各自独立：子进程关闭 fd 3 不影响父进程的 fd 3。不过，两个 fd 指向的是**同一个内核文件对象**（共享文件偏移量和状态标志）。
...
这个返回值里的 `int` 只负责报告调用是否成功：成功返回 `0`，失败返回 `-1` 并设置 `errno`。真正要拿到的两个文件描述符并不在返回值里，也不是某个“管道对象”，而是**由调用者先提供一个长度为 2 的数组**，内核成功后把它们写回这个数组：
...
if (pid1 == 0) {
    ...
    dup2(pipe_fds[1], 1);               // stdout -> pipe write end
    close(pipe_fds[1]);                 // original fd redundant after dup2
...
pid_t pid2 = fork();
if (pid2 == 0) {
    close(pipe_fds[1]);                 // don't need write end
    dup2(pipe_fds[0], 0);               // stdin -> pipe read end
    close(pipe_fds[0]);                 // original fd redundant after dup2
    execl("/bin/grep", "grep", ".zig", (char *)NULL);
    _exit(1);
}

close(pipe_fds[0]);                     // parent uses neither end
close(pipe_fds[1]);                     // otherwise grep never gets EOF
waitpid(pid1, NULL, 0);
waitpid(pid2, NULL, 0);
```
...
从文件描述符的视角看，这里并没有引入新的魔法：shell 只是把 `ls` 的 fd 1 从 tty 改接到管道写端，再把 `grep` 的 fd 0 从 tty 改接到管道读端。于是 `ls` 照常 `write(1, ...)`，只是数据不再去终端，而是进了管道缓冲区；`grep` 也照常 `read(0, ...)`，只是读到的不再是键盘输入，而是管道里的数据。
