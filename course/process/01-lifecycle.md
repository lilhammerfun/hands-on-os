...
sequenceDiagram
    ...
    Shell->>Child: fork() — 复制自己
    Child->>Child: exec("ls") — 替换为 ls
    Child-->>Shell: exit() — 退出
    Shell->>Shell: wait() — 回收子进程，显示下一个提示符
```
...
文件描述符(file descriptor, fd) 是进程访问文件、设备和管道的统一接口——一个非负整数，作为进程文件描述符表的下标。
...
fork()
// 在子进程中：
close(1)                    // 关闭 stdout，fd 1 变空闲
open("output.txt", ...)     // 找到最小空闲 fd → 得到 fd 1
exec("ls")                  // ls 向 fd 1 写输出，实际写入了 output.txt
```
...
└────┴─────────┘                               └────┴─────────┘
        ↕ 独立的 fd 表                                ↕ 独立的 fd 表
  子进程 close(1) 不影响父进程          但两边的 fd 0 指向同一个 PTY
```
...
```zig
const pipe_fds = try posix.pipe();  // 创建管道

// --- 第一个子进程：ls ---
const pid1 = try posix.fork();
if (pid1 == 0) {
    posix.close(pipe_fds[0]);                          // 不需要读端
    try posix.dup2(pipe_fds[1], posix.STDOUT_FILENO);  // stdout → 管道写端
    posix.close(pipe_fds[1]);                          // dup2 后原 fd 多余
    return posix.execvpeZ("ls", &.{"ls"}, std.c.environ);
}
...
if (pid2 == 0) {
    posix.close(pipe_fds[1]);                          // 不需要写端
    try posix.dup2(pipe_fds[0], posix.STDIN_FILENO);   // stdin → 管道读端
    posix.close(pipe_fds[0]);                          // dup2 后原 fd 多余
    return posix.execvpeZ("grep", &.{ "grep", ".zig" }, std.c.environ);
}

// --- 父进程（shell）---
posix.close(pipe_fds[0]);   // 关闭管道两端
posix.close(pipe_fds[1]);   // 否则 grep 的读端永远等不到 EOF
...
| API | 签名 | 说明 |
|-----|------|------|
| `fork` | `fn fork() ForkError!pid_t` | 返回 0 → 子进程，>0 → 父进程 |
| `execvpeZ` | `fn execvpeZ(file, argv_ptr, envp) ExecveError` | 成功不返回；`file` 搜索 PATH；传环境用 `std.c.environ` |
| `waitpid` | `fn waitpid(pid, flags) WaitPidResult` | flags=0 阻塞，`W.NOHANG` 非阻塞 |
| `chdir` | `fn chdir(dir_path: []const u8) ChangeCurDirError!void` | 接受普通切片，不要求 null 结尾 |
| `pipe` | `fn pipe() PipeError![2]fd_t` | `fds[0]` 读端，`fds[1]` 写端 |
| `dup2` | `fn dup2(old_fd, new_fd) !void` | 让 new_fd 成为 old_fd 的副本 |
| `close` | `fn close(fd) void` | 不返回 error |
| `exit` | `fn exit(status: u8) noreturn` | 参数 u8 |
...
`execvpeZ` 要求所有参数都是 null-terminated（`[*:0]const u8` 和 `[*:null]const ?[*:0]const u8`），但 Zig 的 `[]u8` 切片不带 null。**不能直接 `@ptrCast`**。
