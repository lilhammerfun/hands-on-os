...
处置(disposition)决定信号到达时进程怎么响应。每个信号都有一个当前处置。
...
内核用**伪终端(PTY, pseudo-terminal)**——一对虚拟文件描述符——来连接终端模拟器和终端驱动，模拟过去的串口连接。终端模拟器写入 PTY 的一端，终端驱动从另一端读取。
...
物理键盘
  → 键盘驱动（内核）：扫描码 → 按键事件
    → GUI 系统（X11 / Wayland / macOS）：按键事件 → 字符
