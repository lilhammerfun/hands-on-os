...
回答这三个问题需要四个概念。**进程组**把进程绑在一起，实现原子信号投递。**控制终端**记录谁是前台，决定键盘信号的路由。两者结合产生 **Job Control**，即 fg/bg/Ctrl+Z 的完整机制。**会话**是包含多个进程组的更大容器，负责在终端关闭时通知所有进程。最后讨论一个边界情况：**孤儿进程组**。

这些概念构成三级层次：会话 > 进程组 > 进程。控制终端绑在会话上，指向其中一个进程组作为前台。

## 进程组

进程组(process group)是一组共享同一个 PGID(Process Group ID) 的进程。PGID 等于组长(group leader)的 PID。组长就是第一个加入（或创建）这个组的进程。
...
## 动手做一做：zish 进程组与 Job Control

当前 zish 能运行单个命令，能处理 SIGINT。本节实现管道、进程组和 job control，让 zish 支持 `ls | grep foo`、Ctrl+Z、fg/bg。

**本节起 zish 使用 Zig nightly（0.16-dev）开发。**

**API 变化：0.15 → 0.16-dev**

0.16-dev 的标准库重构把 API 分成了三层：

| 层 | 模块 | 特点 |
|----|------|------|
| 高级 I/O | `std.Io` | 支持 threaded/io_uring/kqueue 多后端 |
| 信号 / 终端控制 | `std.posix` | 高级错误处理封装（正在被清空，但信号相关还在） |
| 原始 syscall | `std.os.linux` | 返回 `usize`，负值表示 errno，无错误处理 |

关键变化：

| 0.15 | 0.16-dev | 说明 |
|------|----------|------|
| `std.fs.File` | `std.Io.File` | `std.fs` 已弃用 |
| `file.reader(&buf)` | `file.reader(io, &buf)` | 需要 `Io` 上下文 |
| `reader.interface.print(...)` | `reader.print(...)` | 不再需要 `.interface` |
| `std.posix.fork()` | `std.os.linux.fork()` | 返回 `usize` |
| `std.posix.execvpeZ(...)` | 已删除 | 用 `std.os.linux.execve()` + 手动搜 PATH |
| `std.posix.pipe()` | `std.os.linux.pipe(&fds)` | 参数改为指针 |
| `std.posix.dup2(old, new)` | `std.os.linux.dup2(old, new)` | 返回 `usize` |
| `std.posix.close(fd)` | `std.os.linux.close(fd)` | 返回 `usize` |
| `std.posix.waitpid(pid, flags)` | `std.os.linux.waitpid(pid, &status, flags)` | status 改为指针参数 |
| `std.posix.setpgid(pid, pgid)` | `std.os.linux.setpgid(pid, pgid)` | 返回 `usize` |
| `std.posix.exit(u8)` | `std.os.linux.exit(i32)` | 参数类型变了 |
| `std.posix.sigaction(...)` | `std.posix.sigaction(...)` | **不变** |
| `std.posix.tcsetpgrp(...)` | `std.posix.tcsetpgrp(...)` | **不变** |
| `posix.SIG.INT` (u8) | `posix.SIG.INT` (enum) | SIG 从整数变为 enum |

**API 速查**

**低级 syscall 封装**（`std.os.linux`）：
...
`execvpeZ` 已删除。`execve` 不搜 PATH，需要传完整路径。zish 要自己实现 PATH 搜索。
...
```zig
posix.sigaction(posix.SIG.INT, &ign, null);    // Ctrl+C 不杀 shell
posix.sigaction(posix.SIG.TSTP, &ign, null);   // Ctrl+Z 不停 shell
posix.sigaction(posix.SIG.TTIN, &ign, null);   // 后台组时不被停
posix.sigaction(posix.SIG.TTOU, &ign, null);   // 调 tcsetpgrp 时不被停
```
...
for (0..cmd_count) |i| {
    // 非最后一个命令：创建管道
    // fork
    // 子进程：setpgid → 管道重定向 → 重置信号 → exec
    // 父进程：setpgid（双保险）→ 关闭不需要的管道端
}

// 设为前台 → 等待所有子进程 → 收回前台
```

子进程四个步骤（进程组 → 管道重定向 → 重置信号 → exec）和本章的 Shell 管道流程图一一对应。
