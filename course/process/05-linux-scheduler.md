...
**核心洞察**：调度算法本质上都在做同一个权衡：吞吐量（少切换、长时间片）和响应时间（多切换、短时间片）之间的取舍。经典算法从 FCFS 到 MLFQ，每一步都是在解决前一步留下的问题。CFS/EEVDF 把这个权衡转化为一个记账问题：追踪每个进程用了多少 CPU，始终让用得最少的先运行，不需要手动分级和调参。实时调度则跳出了这个权衡，用确定性保证取代公平性追求。多核调度在此基础上增加了空间维度：不仅要决定"下一个运行谁"，还要决定"在哪个核心上运行"。
