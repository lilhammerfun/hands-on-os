...
对于一个从未接受过计算机科班教育，也没有体系地学过操作系统的我，这件事有些复杂，也大概率会闹很大笑话。但我很认真，并且我认为这不是我写这本书的阻碍。恰恰相反，一个没见过操作系统全貌的人，反而更清楚什么地方真正让初学者困惑，哪些解释需要再多走一步，哪些"显而易见"的跳跃其实一点都不显而易见。这本书就是这个过程的产物：一个同样在学习的人，把自己搞懂的东西整理出来。需要更加审慎和敬畏的是，尊重前人们在OS的宝贵实践，尊重源码。
...
全书分为两大部分：9 章理论课（31 课）和 4 个项目（13 课），合计 44 课。理论课提供完整的知识体系，项目课独立成章，可回引多篇理论课。

**理论部分**

**基础与概览**（2 课）从操作系统的全景开始，介绍计算机体系结构、内核结构，然后深入系统调用机制和用户态/内核态的双模式运行。

**进程管理**（7 课）是篇幅最大的一章。从进程的创建和销毁讲起，经过信号、进程组与会话、CPU 调度算法、Linux 调度器实现，到命名空间（Namespace）和 Cgroups 的资源隔离。

**线程与并发**（5 课）讨论线程模型、同步原语（锁、信号量、条件变量）、死锁问题、事件驱动并发模型，以及 Linux 内核自身使用的同步机制（RCU、Seqlock 等）。

**内存管理**（4 课）覆盖地址空间与分页、虚拟内存、内存映射，以及内核内存分配器（伙伴系统、Slab）。

**网络与 IPC**（3 课）从 Socket 编程和 TCP 协议开始，讨论进程间通信（IPC）的各种机制，最后进入零拷贝和高性能网络技术。

**存储与 I/O**（3 课）介绍 I/O 系统的分层架构、中断与设备驱动模型、块层和 I/O 调度器。

**文件系统**（4 课）从文件与目录的基本抽象出发，深入 VFS 实现、日志机制、崩溃一致性。

**保护与安全**（1 课）集中讨论保护环、访问控制、Linux Capabilities、Seccomp 和内存安全机制。

**虚拟化**（1 课）讲解硬件虚拟化原理、KVM 架构和 Virtio 半虚拟化，并回顾容器与虚拟机的区别。

**可观测性**（1 课）介绍 perf、ftrace、eBPF 等内核追踪工具，学习如何在生产环境中诊断性能问题。

**项目部分**

四个项目各自独立成章，每个项目包含多课，可以在理论课学完对应前置知识后随时开始。

**zish：Shell**（3 课）从基础 REPL 开始，逐步加入 Job Control，最后实现 Namespace 隔离并移植到自己写的内核。

**zedis：内存数据库**（4 课）从事件循环和 RESP 协议解析开始，加入 fork-COW 持久化、TCP 服务器优化，最后用 perf 和火焰图做性能分析。

**zcryptfs：加密文件系统**（3 课）从 FUSE 用户态文件系统开始，加入透明加密层，最后确保崩溃一致性。

**zigos：裸机操作系统**（4 课）从引导和物理内存管理开始，依次实现中断处理、进程调度、系统调用、文件系统和设备驱动。
