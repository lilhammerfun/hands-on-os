...
这本书反过来。整个学习过程围绕 4 个真实项目展开。每个项目都是一个可用的系统，不是教学玩具。操作系统的概念在构建这些系统的过程中引入，遇到什么问题就学什么知识。调度、内存、文件系统这些主题不是按教科书章节排列的，而是在项目需要它们的时候出现。

理论课和项目课穿插进行。全书 33 课中，约一半是纯理论课，提供完整的知识体系。另一半是项目课，在动手构建的过程中把理论变成可运行的代码。
...
**进程管理**（7 课）是篇幅最大的一章。从进程的创建和销毁讲起，经过信号、进程组与会话、CPU 调度算法、Linux 调度器实现，到命名空间（Namespace）和 Cgroups 的资源隔离。zish 项目在这一章启动。

**线程与并发**（5 课）讨论线程模型、同步原语（锁、信号量、条件变量）、死锁问题、事件驱动并发模型，以及 Linux 内核自身使用的同步机制（RCU、Seqlock 等）。zedis 项目在这一章启动。
...
**文件系统**（4 课）从文件与目录的基本抽象出发，深入 VFS 实现、日志机制、崩溃一致性。zcryptfs 项目在这一章推进。

**保护与安全**（1 课）集中讨论保护环、访问控制、Linux Capabilities、Seccomp 和内存安全机制。

**虚拟化**（1 课）讲解硬件虚拟化原理、KVM 架构和 Virtio 半虚拟化，并回顾容器与虚拟机的区别。

**从裸机到操作系统**（4 课）是 zigos 项目的主战场。从引导和物理内存管理开始，依次实现中断处理、进程调度、系统调用、文件系统和设备驱动。

最后还有一课**可观测性**，介绍 perf、ftrace、eBPF 等内核追踪工具，学习如何在生产环境中诊断性能问题。
